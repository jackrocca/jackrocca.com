...
6. Wednesday 09:00 **America/Los_Angeles**, the first successful sync freezes the DraftKings lines and opens picks (normally within five minutes); Saturday 09:00 the sync refreezes the Sunday and Monday games and moves saved picks onto those final numbers. A commissioner can run either snapshot earlier. A game's line cannot be changed once its snapshot has run. If the feed is unavailable, publication retries until the Sunday deadline; a game that has kicked off never receives a line from potentially in-play odds.
...
- Deadline = the first Sunday main-slate kickoff of that week (normally 10:00 Pacific), computed from the schedule, **not a hardcoded time**. Games that kick off earlier (Thursday, Friday/Saturday, 06:30 Pacific international games) are optional and lock only their own slot at kickoff; the rest of the card stays editable until Sunday. Powerups lock with the slot they affect.
- Existing entries lock at that deadline. A new late entry may select only four unstarted games, gets a one-point penalty (floor zero), cannot use any powerup, and locks immediately. Lateness is derived on read from the save time and the deadline.
- Another member's slot is visible from its game's kickoff or the deadline, whichever comes first. See `docs/architecture.md` for the two line snapshots (Wednesday for early games, Saturday for Sunday and Monday games).
