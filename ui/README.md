...
Local adaptations include structured children for unstyled buttons, labeled form selects, disabled mobile choices, native drawer trigger semantics, stable reselection, property-specific transitions, and reduced-motion support. `TabPanels` is ported onto a Base UI `tabs` primitive without the upstream Motion dependency: the underline slides with `transform` only, panels fade in with `@starting-style`, and keyboard activation sets `data-instant` so nothing animates for it. Easing tokens (`--ease-out`, `--ease-in-out`, `--ease-drawer`) live in `theme.css`. Preserve upstream provenance; see `THIRD_PARTY_NOTICES.md`.
