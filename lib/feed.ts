          ...
          z.object({
            ...
            pointSpread: z
              .object({
                home: z
                  .object({
                    close: z.object({ line: z.string().optional() }).optional(),
                  })
                  .optional(),
              })
              .optional(),
...
export function parseFeed(raw: unknown, week: number): Game[] {
  ...
  if (
    ...
    throw new Error("Unexpected season or week from schedule provider.");
  return data.events
    .map((e) => {
      if (
        e.season.year !== 2026 ||
        ...
        throw new Error("Invalid game season/date.");
      ...
      const state: Game["state"] = /CANCEL|FORFEIT/.test(s.name)
        ? "canceled"
        : /POSTPON|SUSPEND|DELAY/.test(s.name)
          ? "postponed"
          : s.completed
            ? "final"
            : s.state === "in"
      ...
      return {
        ...
        week,
        kickoff: e.date,
        timeConfirmed: c.timeValid !== false,
        home: team(h),
        away: team(a),
