// The jackrocca.com account directory. Accounts bind to Google's stable `sub`;
// Pick 4 membership is a league concern layered on top (auto-enrolled for 2026).
// For the 2026 season the directory lives in the league Blob's `state.users`.
import { z } from "zod";
...
export function findAccount(state: State, id: string): User | undefined {
  return state.users.find((u) => u.id === id);
}
// The account fields a signed-in person may see about themselves. Never
// includes Google identifiers or session material.
...
export function signInWithGoogle(state: State, raw: unknown, ownerEmail: string): User {
